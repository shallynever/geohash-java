...
public class WGS84PointTest {
    ...
    @Test
    public void testVincenty45() {
        List<WGS84Point> pointList = new ArrayList<>(10);
        for (int i = 0; i < 10; i++) {
            pointList.add(new WGS84Point(40 + i, 102 + i));
        }
        pointList.add(0, new WGS84Point(50, 111));
        List<Double> latitudeSet = pointList.stream().map(WGS84Point::getLatitude).collect(Collectors.toList());
        for (Double aDouble : latitudeSet) {
